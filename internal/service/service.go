...
func (s *Service) ReevaluateWindow(windowID int64) (*model.SampleWindow, error) {
	...
	prevHash, recentHashes, err := s.windows.ReevaluationEvidence(win.SourceID, win.Seq)
	if err != nil {
		return nil, err
	}
	replay := health.DetectHashRepetition(win.SampleHash, prevHash, recentHashes)
	allPass := true
	firstFail := ""
	for _, t := range tests {
		if !t.Passed {
	...
	if replay.IsReplay {
		status = model.WindowStatusRepeatAnomaly
		anomaly = model.AnomalyReplay
	}
