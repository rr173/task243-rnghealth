...
// LatestValidSeqAfter returns the latest valid window after a restart boundary.
func (w *WindowStore) LatestValidSeqAfter(sourceID, afterSeq int64) (int64, error) {
	var seq int64
	err := w.db.QueryRow(
		`SELECT COALESCE(MAX(seq), 0) FROM sample_windows WHERE source_id = ? AND status = ?`,
		sourceID, model.WindowStatusValid,
