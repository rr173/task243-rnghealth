...
func (e *EventStore) SetState(id int64, state string, confirmedAt, closedAt *time.Time) error {
	_, err := e.db.Exec(
		`UPDATE health_events SET state = ?, confirmed_at = CASE WHEN ? = ? THEN ? ELSE NULL END, closed_at = ? WHERE id = ?`,
		state, state, model.EventStateConfirmed, nullTimeVal(confirmedAt), nullTimeVal(closedAt), id)
	return mapErr(err)
}
...
func (e *EventStore) UpdateRun(id, firstSeq, lastSeq int64, windowCount int, anomalyType string, afterRestart bool) error {
	_, err := e.db.Exec(
		...
		firstSeq, lastSeq, windowCount, anomalyType, boolToInt(afterRestart), id)
	return mapErr(err)
}
